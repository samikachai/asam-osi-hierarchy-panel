...
function GroundTruthHierarchyPanel({ context }: { context: PanelExtensionContext }): ReactElement {
  const { defaultSelected, items, renderDone, setCurrentFilter, setDefaultSelected, handleChange, metaData } = useGroundTruthHierarchy();
  useLayoutEffect(() => {
  ...
  const renderTree = (nodes: RenderTree) =>
    ...
    nodes.children.map((node) => (
      <TreeView defaultSelected={defaultSelected} onSelectedChanged={setDefaultSelected} key={node.name} node={node} />
    ));
